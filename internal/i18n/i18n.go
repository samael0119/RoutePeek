...
var dict = map[string]map[string]string{
	"zh": {
		...
		"diag_gw_conflict_title": "检测到网关路由冲突",
		"diag_gw_conflict_msg":   "网关 %s 有多个相同 metric 的路由，可能导致路由不稳定。",
	...
	"en": {
		...
		"diag_gw_conflict_title": "Gateway Route Conflict Detected",
		"diag_gw_conflict_msg":   "Gateway %s has multiple routes with same metric. May cause instability.",
